...
Trajet*InsertTete(Trajet*L,Trajet t)
{
   Trajet*T;
    T=(Trajet*)malloc(sizeof(Trajet));
...
Trajet*Milieu(Trajet*L,Trajet*k,Trajet t)//insere apres le compteur k
{
    Trajet*p;
    p=L;
    while((p!=NULL)&&(p!=k))
    ...
    if(p==NULL)
    {
        printf("Pointeur inexistant dans la liste\n\n");
        return L;
    }
    else
    {
        Trajet *m;
        m=(Trajet*)malloc(sizeof(Trajet));
        *m=t;
        m->next=p->next;
        p->next=m;
    }
